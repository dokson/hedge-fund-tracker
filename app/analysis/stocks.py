...
def aggregate_quarter_by_fund(df_quarter) -> pd.DataFrame:
    ...
    df_quarter = (
        df_quarter.drop(columns=["Ticker", "Company"])
        .set_index("CUSIP")
        .join(df_stocks[["Ticker", "Company"]], how="left")
        .reset_index()
    )
    ...
    df_fund_quarter.loc[
        (df_fund_quarter["Portfolio_Pct"] == 0) & (df_fund_quarter["Shares"] > 0), "Portfolio_Pct"
    ] = 0.009

    # 1. Calculate Portfolio_Pct_Rank for each stock within each fund
    df_fund_quarter["Portfolio_Pct_Rank"] = df_fund_quarter.groupby("Fund")["Portfolio_Pct"].rank(
        ascending=False, method="min"
    )

    # 2. Calculate Fund_Concentration_Ratio (Sum of Portfolio_Pct for top 10 holdings) for each fund
    concentration_map = (
        df_fund_quarter[df_fund_quarter["Portfolio_Pct_Rank"] <= 10]
        .groupby("Fund")["Portfolio_Pct"]
        .sum()
        .to_dict()
    )
    ...
    df_fund_quarter["Delta"] = df_fund_quarter.apply(
        lambda row: (
            ...
            if row["Shares"] > 0 and row["Shares"] == row["Delta_Shares"]
            else format_percentage(
                row["Delta_Shares"] / (row["Shares"] - row["Delta_Shares"]) * 100, True
            )
        ),
        axis=1,
    )
...
def _calculate_fund_level_flags(df_fund_quarter: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_high_conviction"] = df["is_new"] & (
        (df["Portfolio_Pct_Rank"] <= 10) | (df["Portfolio_Pct"] > 3.0)
    )
...
def _aggregate_stock_data(df_fund_quarter_with_flags: pd.DataFrame) -> pd.DataFrame:
    ...
    df_agg = (
        df_fund_quarter_with_flags.groupby(["Ticker", "Company"])
    ...
    mask_accumulation = (
        df_fund_quarter_with_flags["is_buyer"] & ~df_fund_quarter_with_flags["is_new"]
    )
    avg_delta_map = (
        df_fund_quarter_with_flags[mask_accumulation]
        .groupby(["Ticker"])["Shares_Delta_Pct"]
        .mean()
        .to_dict()
    )
...
def _calculate_derived_metrics(df_analysis: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Buyer_Seller_Ratio"] = np.where(
        df["Seller_Count"] > 0, df["Buyer_Count"] / df["Seller_Count"], np.inf
    )

    previous_total_value = np.where(
        df["Total_Value"] - df["Total_Delta_Value"] == 0,
        np.nan,
        df["Total_Value"] - df["Total_Delta_Value"],
    )
    df["Delta"] = np.where(
        (df_analysis["New_Holder_Count"] == df_analysis["Holder_Count"])
        & (df_analysis["Close_Count"] == 0),
        np.inf,
        df_analysis["Total_Delta_Value"] / previous_total_value * 100,
    )
