...
import DueDiligenceReportList from "@/components/DueDiligenceReportList";
...
export default function AIDueDiligence() {
  ...
  const { data: allSavedReports = [], refetch: refetchReports } = useQuery({
    queryKey: ["due-diligence-reports"],
    queryFn: () => getDueDiligenceReports(),
    enabled: true,
    staleTime: 0,
    refetchOnMount: true,
  ...
  const savedReports = useMemo(() => {
    if (!ticker) return [];
    return allSavedReports.filter((r) => r.ticker === ticker.toUpperCase());
  }, [allSavedReports, ticker]);

  const reportsByTicker = useMemo(() => {
    const grouped = new Map<string, typeof allSavedReports>();
    for (const report of allSavedReports) {
      if (!report.ticker) continue;
      if (!grouped.has(report.ticker)) {
        grouped.set(report.ticker, []);
      }
      grouped.get(report.ticker)!.push(report);
    }
    return grouped;
  }, [allSavedReports]);
  ...
  const { data: holdings = [] } = useQuery({
    ...
    staleTime: 10 * 60 * 1000,
    enabled: !!ticker && stocks.length > 0 && validTickers.has(ticker),
  });
      ...
      {report && !loading && (
        <div className="animate-slide-up space-y-5">
          ...
          {savedReports.length > 0 && (
            <DueDiligenceReportList
              reports={savedReports}
              title={`Historical Reports for ${ticker.toUpperCase()}`}
              ...
              {ticker ? (
                savedReports.length > 0 ? (
                  ...
                  <div className="text-center text-muted-foreground text-sm py-4">
                    No saved reports for {ticker.toUpperCase()}
                  </div>
                )
              ) : (
                <div className="space-y-4">
                  {[...reportsByTicker.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([tickerKey, reports]) => (
                    <div key={tickerKey}>
                      <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                        <span className="text-primary">{tickerKey}</span>
